...
map_load_container = async (path) => {
	...
	*/
	...
	let data = window.Q1K3_PACKED && path.slice(-1) == 'l' ?
			window.Q1K3_PACKED.l :
...
map_init = (m) => {
	...
	let spawn_class = [
		...
		/* 16 */ entity_torch_t,
		/* 17 */ entity_player_dummy_t,
		/* 18 */ entity_jump_pad_t,
	];

	// Parse entity data and spawn all entities for this map
	for (let i = 0; i < map.e.length;) {
		let type = spawn_class[m.e[i++]];
		if (window.LNQ1_MULTIPLAYER && type === entity_player_dummy_t) {
			i += 5;
			continue;
		}
		let pos = vec3(m.e[i++] * 32, m.e[i++] * 16, m.e[i++] * 32),
			p1 = m.e[i++],
			p2 = m.e[i++];
		if (window.LNQ1_MULTIPLAYER && type === entity_player_t && window.LNQ1_MULTIPLAYER.spawnPoint) {
			pos = window.LNQ1_MULTIPLAYER.spawnPoint(pos);
			pos = map_find_open_spawn(pos);
		}
...
map_find_open_spawn = (preferred) => {
	...
	for (let r of radii) {
		for (let oy of y_offsets) {
			let candidates = r ?
				[
					vec3(preferred.x + r, preferred.y + oy, preferred.z),
					vec3(preferred.x - r, preferred.y + oy, preferred.z),
					vec3(preferred.x, preferred.y + oy, preferred.z + r),
					vec3(preferred.x, preferred.y + oy, preferred.z - r),
					vec3(preferred.x + r, preferred.y + oy, preferred.z + r),
					vec3(preferred.x - r, preferred.y + oy, preferred.z + r),
					vec3(preferred.x + r, preferred.y + oy, preferred.z - r),
					vec3(preferred.x - r, preferred.y + oy, preferred.z - r)
				] :
				[vec3(preferred.x, preferred.y + oy, preferred.z)];
			for (let p of candidates) {
				if (p.x > 64 && p.z > 64 && p.x < 4032 && p.z < 4032 && is_open(p)) {
					return p;
				}
			}
		}
...
window.map_find_open_spawn = map_find_open_spawn;
