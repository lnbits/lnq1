...
game_load = async () => {
	...
	let asset_base = window.Q1K3_ASSET_BASE || /*DEBUG[*/ 'build/' + /*]*/ '';
	map_data = await map_load_container(asset_base + 'l');
	model_data = await model_load_container(asset_base + 'm');
	...
	let ln_start = r_num_verts,
		ln_block = (x, y, w, h) => r_push_block(x, y, -1.5, w, h, 3, 1);
	ln_block(-5.5, -2, 1.4, 8);
	ln_block(-5.5, -2, 5.2, 1.4);
	ln_block(1, -2, 1.4, 8);
	ln_block(6, -2, 1.4, 8);
	ln_block(2.2, 3.8, 1.4, 1.8);
	ln_block(3.4, 2.2, 1.4, 1.8);
	ln_block(4.6, 0.6, 1.4, 1.8);
	model_ln = {f: [ln_start], nv: r_num_verts - ln_start};
	...
	model_jump_pad = model_init(model_data[5], 3.5,0.12,3.5);
	...
	let game_started = 0,
		...
		capture_mouse = () => {
			...
			if (capture_prompt) {
				capture_prompt.remove();
				capture_prompt = 0;
			}
		},
		show_capture_prompt = () => {
			if (document.pointerLockElement === c || capture_prompt) return;
			capture_prompt = document.createElement('div');
			capture_prompt.id = 'join';
			capture_prompt.innerHTML = '<form><h2>READY</h2><button>CLICK TO CAPTURE MOUSE</button></form>';
			g.appendChild(capture_prompt);
			capture_prompt.onclick = (ev) => {
				ev.preventDefault();
				ev.stopPropagation();
				capture_mouse();
			};
		...
		unlock_game = () => {
			...
			g.onclick = capture_mouse;
			capture_mouse();
			setTimeout(show_capture_prompt, 250);
			if (!sounds_ready) {
				sounds_ready = 1;
				audio_init();
	...
	document.addEventListener('pointerlockchange', () => {
		if (document.pointerLockElement === c && capture_prompt) {
			capture_prompt.remove();
			capture_prompt = 0;
		}
	});
	...
	*/
	if (
		...
		start_game(1);
		ui_show_join(unlock_game);
	}
	else {
		g.onclick = () => ui_show_join(() => start_game());
	}
},
...
run_frame = (time_now) => {
	...
	r_draw(
		vec3(-1.2, 4.8, -1), 0, 0, 1,
		model_ln.f[0], model_ln.f[0], 0,
		model_ln.nv
	);
	r_push_light(
