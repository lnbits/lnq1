...
class entity_player_t extends entity_t {
	_init() {
		...
		if (window.LNQ1_SPAWN_YAW != null) {
			this._yaw = window.LNQ1_SPAWN_YAW;
		}
		this._bob = 0;

		game_entity_player = this;
		window.game_entity_player = this;
	...
	_update() {
		...
		) && (!window.LNQ1_MULTIPLAYER || !window.LNQ1_MULTIPLAYER.canPlay());
		...
		this.a = vec3_mulf(
			vec3_rotate_y(
				vec3(
					controls_locked ? 0 : keys[key_right] - keys[key_left],
					0,
					controls_locked ? 0 : keys[key_up] - keys[key_down]
				), 
				this._yaw
			), 
		...
		if (!controls_locked && keys[key_jump] && this._on_ground && this._can_jump) {
			this.v.y = 400;
			this._on_ground = 0;
			this._can_jump = 0;
		}
		...
		// Shoot Weapon
		if (!controls_locked && keys[key_action] && shoot_wait < 0) {
	...
	_kill() {
		...
		if (window.LNQ1_MULTIPLAYER) {
			window.LNQ1_MULTIPLAYER.onLocalDeath?.();
			return;
		}
