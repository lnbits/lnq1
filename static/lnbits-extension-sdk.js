;(function () {
  ...
  const LOG_PREFIX = '[streetfighterwasm extension]'
  ...
  function createLNbitsExtensionClient({extensionId}) {
    ...
    return {
      ...
      publishStart(gameId, payload) {
        return request(`${baseUrl}/games/${encodeURIComponent(gameId)}/start`, {
          method: 'POST',
          body: payload
        })
      },

      publishInput(gameId, payload) {
        return request(`${baseUrl}/games/${encodeURIComponent(gameId)}/input`, {
          method: 'POST',
          body: payload
        })
      },
      ...
      publishState(gameId, payload) {
        return request(`${baseUrl}/games/${encodeURIComponent(gameId)}/state`, {
          method: 'POST',
          body: payload
        })
      },

      publishChat(gameId, payload) {
        return request(`${baseUrl}/games/${encodeURIComponent(gameId)}/chat`, {
          method: 'POST',
          body: payload
        })
      },
  ...
  function subscribeWebsocket(itemId, callback) {
    ...
    return bridgeRequest({
      ...
      .then(() => {
        ...
        unsubscribe.send = data => bridgeRequest({
